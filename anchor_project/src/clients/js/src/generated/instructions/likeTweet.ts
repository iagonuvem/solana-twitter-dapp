...
export const LIKE_TWEET_DISCRIMINATOR = new Uint8Array([
  248, 27, 137, 254, 228, 130, 141, 149,
]);

export function getLikeTweetDiscriminatorBytes() {
  return fixEncoderSize(getBytesEncoder(), 8).encode(LIKE_TWEET_DISCRIMINATOR);
}

export type LikeTweetInstruction<
...
export type LikeTweetInstructionData = { discriminator: ReadonlyUint8Array };

export type LikeTweetInstructionDataArgs = {};

export function getLikeTweetInstructionDataEncoder(): FixedSizeEncoder<LikeTweetInstructionDataArgs> {
  return transformEncoder(
    getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)]]),
    (value) => ({ ...value, discriminator: LIKE_TWEET_DISCRIMINATOR })
  );
}

export function getLikeTweetInstructionDataDecoder(): FixedSizeDecoder<LikeTweetInstructionData> {
  return getStructDecoder([
    ['discriminator', fixDecoderSize(getBytesDecoder(), 8)],
  ]);
}

export function getLikeTweetInstructionDataCodec(): FixedSizeCodec<
  LikeTweetInstructionDataArgs,
  LikeTweetInstructionData
> {
  return combineCodec(
    getLikeTweetInstructionDataEncoder(),
    getLikeTweetInstructionDataDecoder()
  );
}

export type LikeTweetAsyncInput<
...
export async function getLikeTweetInstructionAsync<
  TAccountReactionAuthor extends string,
  TAccountTweetReaction extends string,
  TAccountTweet extends string,
  TAccountSystemProgram extends string,
...
> {
  ...
  const instruction = {
    ...
    programAddress,
    data: getLikeTweetInstructionDataEncoder().encode({}),
  } as LikeTweetInstruction<
    TProgramAddress,
    TAccountReactionAuthor,
    TAccountTweetReaction,
    TAccountTweet,
...
export function getLikeTweetInstruction<
  TAccountReactionAuthor extends string,
  TAccountTweetReaction extends string,
  TAccountTweet extends string,
  TAccountSystemProgram extends string,
...
export type ParsedLikeTweetInstruction<
  TProgram extends string = typeof TWITTER_PROGRAM_ADDRESS,
...
> = {
  ...
  data: LikeTweetInstructionData;
};

export function parseLikeTweetInstruction<
...
>(
  instruction: Instruction<TProgram> &
    ...
    InstructionWithData<ReadonlyUint8Array>
): ParsedLikeTweetInstruction<TProgram, TAccountMetas> {
  ...
  return {
    ...
    data: getLikeTweetInstructionDataDecoder().decode(instruction.data),
  };
}
